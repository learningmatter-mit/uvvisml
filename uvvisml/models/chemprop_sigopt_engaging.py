...
--ensemble_size 1"""
    ...
    if args.separate_val_path and args.separate_test_path:
        run_command = (
            run_command
            + f" --separate_val_path {args.separate_val_path} --separate_test_path {args.separate_test_path}"
        )
    if args.solvation:
    ...
    if args.separate_val_features_path and args.separate_test_features_path:
        run_command = (
            run_command
            + f" --separate_val_features_path {args.separate_val_features_path} --separate_test_features_path {args.separate_test_features_path}"
        )
    ...
    final_val_score = np.mean(
        [
            float(y.split()[6])
            for y in [x for x in results if f"best validation {args.metric}" in x]
        ]
    )
...
def get_parser():
    parser = argparse.ArgumentParser(
        description="Runs SigOpt to optimize hyperparameters of a Chemprop model"
    )
    parser.add_argument(
        "--data_path",
        type=str,
        default="smiles_target_train.csv",
        help="training data (if validation and test files are specified) or all data (if no validation and test files are specified)",
    )
    parser.add_argument(
        "--separate_val_path", type=str, default=None, help="validation data"
    )
    parser.add_argument(
        "--separate_test_path", type=str, default=None, help="test data"
    )
    parser.add_argument(
        "--features_path", type=str, default=None, help="path to features file"
    )
    parser.add_argument(
...
def main(args=None):
    ...
    sigopt_api_key = os.environ.get(
        "SIGOPT_API_KEY"
    )  # private API key is stored in a local environment variable
    ...
    if args.restart:
        ...
        print(
            "Restarting experiment: https://app.sigopt.com/experiment" + experiment.id
        )
        ...
        experiment = conn.experiments().create(
            ...
            project="fluodye",
            parameters=[
                dict(name="hidden_size", type="int", bounds=dict(min=300, max=1200)),
                ...
                dict(
                    name="ffn_hidden_size", type="int", bounds=dict(min=300, max=1200)
                ),
                dict(name="warmup_epochs", type="int", bounds=dict(min=2, max=6)),
                dict(name="batch_size", type="int", bounds=dict(min=10, max=100)),
                dict(name="epochs", type="double", bounds=dict(min=20, max=100)),
                dict(
                    name="init_lr",
                    type="double",
                    bounds=dict(min=1e-3, max=1),
                    transformation="log",
                ),
                dict(
                    name="max_lr",
                    type="double",
                    bounds=dict(min=1e-8, max=1e-3),
                    transformation="log",
                ),
                dict(
                    name="final_lr",
    ...
    best_assignments["init_lr"] = (
        best_assignments["init_lr"] * best_assignments["max_lr"]
    )
    best_assignments["final_lr"] = (
        best_assignments["final_lr"] * best_assignments["max_lr"]
    )

    print("Best Assignments: " + str(best_assignments))
    print(
        "Explore your experiment: https://app.sigopt.com/experiment/"
        + experiment.id
        + "/analysis"
    )
